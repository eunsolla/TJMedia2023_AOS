...
import com.verse.app.databinding.FMyPageBookmarkRootBinding
 ...
 */
@AndroidEntryPoint
class MyPageBookmarkTabFragment :
    BaseFragment<FMyPageBookmarkRootBinding, MyPageBookmarkTabFragmentViewModel>() {
    override val layoutId: Int = R.layout.f_my_page_bookmark_root
    override val viewModel: MyPageBookmarkTabFragmentViewModel by viewModels()
    ...
    inner class PagerAdapter : FragmentStateAdapter(this) {
        ...
        override fun createFragment(position: Int): Fragment {
            return if (position == 0) {
                MyPageBookmarkFeedFragment.newInstance(arguments)
            } else {
                MyPageBookmarkAccompaniment.newInstance(arguments)
            }
        }
    }
    ...
    private fun showFilterDialog(pos: Int) {
        // 공연
        if (pos == 0) {
            val fragment = getFragment(pos, binding.vp) as? MyPageBookmarkFeedFragment
            if (fragment != null) {
            ...
            // 개인 소장
            val fragment = getFragment(pos, binding.vp) as? MyPageBookmarkAccompaniment
            if (fragment != null) {
                val currentSort = fragment.viewModel.getCurrentSort()
                FilterDialogFragment()
                    .setBtnOneName(
    ...
    companion object {
        fun newInstance(bundle: Bundle?): Fragment {
            return MyPageBookmarkTabFragment().apply {
                arguments = bundle
            }
        }
    }
