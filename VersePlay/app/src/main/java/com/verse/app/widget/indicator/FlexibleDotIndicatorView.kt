 ...
 */
class FlexibleDotIndicatorView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        context.obtainStyledAttributes(attrs, R.styleable.FlexibleDotIndicatorView).run {
            isInfinite =
                getBoolean(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorIsInfinite, false)
            var width = getDimensionPixelSize(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorEnableWidth,50)
            var height = getDimensionPixelSize(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorEnableHeight,50)
            enableModel.apply {
                corner =
                    getDimension(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorEnableCorner, -1F)
                paint.color =
                    getColor(
                        R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorEnableColor,
                        Color.BLACK
                    )
                rect.right = width
                rect.bottom = height
            }
            width = getDimensionPixelSize(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorDisableWidth,50)
            height = getDimensionPixelSize(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorDisableHeight,50)
            disableModel.apply {
                corner = getDimension(
                    R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorDisableCorner,
                    -1F
                )
                paint.color =
                    getColor(
                        R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorDisableColor,
                        Color.WHITE
                    )
                rect.right = width
                rect.bottom = height
            ...
            divider = getDimension(R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorDivider, 0F)
            gravity = Gravity.values()[getInt(
                R.styleable.FlexibleDotIndicatorView_flexibleDotIndicatorGravity,
                0
            )]
            recycle()
        }
    ...
    private fun drawDots(canvas: Canvas, point: Int, model: Model) {
        ...
        if (model.corner > 0F) {
            canvas.drawRoundRect(pointRect.toRectF(), model.corner, model.corner, model.paint)
        } else {
    ...
    private fun getContentsWidth(): Int {
        ...
        val disableWidth = (disableModel.rect.width() * Math.max(getRealItemCount().minus(1), 0))
        val dividerSize = divider * Math.max(getRealItemCount().minus(1), 0)
