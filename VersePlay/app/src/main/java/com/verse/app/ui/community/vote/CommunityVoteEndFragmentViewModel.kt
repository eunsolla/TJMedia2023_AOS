...
import com.verse.app.model.param.VoteQueryMap
 ...
 */
@HiltViewModel
class CommunityVoteEndFragmentViewModel @Inject constructor(
    ...
    private val queryMap: VoteQueryMap by lazy { VoteQueryMap().apply { isExpired = true } }
    // [e] Parameter

    fun start() {
        apiService.fetchVotes(queryMap)
            .map { it.result.list }
            .map { getParticipationList(it) }
            .applyApiScheduler()
            .request(success = { handleOnSuccess(it) }, failure = { handleOnError(it) })
    }

    private fun getParticipationList(list: List<CommunityVoteData>): List<CommunityVoteData> {
        return list.map {
            it.isVoteEnd = true
            it
        }
     ...
     * 반주음 API 요청하는 함수
     */
    private fun reqList(queryMap: VoteQueryMap) {
        apiService.fetchVotes(queryMap)
