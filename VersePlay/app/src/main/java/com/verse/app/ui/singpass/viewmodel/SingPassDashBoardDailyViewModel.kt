 ...
 */
@HiltViewModel
class SingPassDashBoardDailyViewModel @Inject constructor(
    ...
    private val _isLoadingFinish: SingleLiveEvent<Unit> by lazy { SingleLiveEvent() }
    val isLoadingFinish: LiveData<Unit> get() = _isLoadingFinish
            ...
            apiService.requestSkipMission(
                ...
                .request({ res->
                    if (res.status == HttpStatusType.SUCCESS.status) {
                        ...
                        requestSingPassMissionInfo(TabPageType.DAILY_MISSION)
                    } else {
